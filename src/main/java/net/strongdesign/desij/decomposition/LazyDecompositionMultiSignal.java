 ...
 */
public class LazyDecompositionMultiSignal extends AbstractDecomposition {
	
	public LazyDecompositionMultiSignal(String filePrefix) {
		super(filePrefix);
	}
	...
	public List<Transition> reduce(STGInOutParameter stgParam) throws STGException {
		...
		while (true) {
			...
			//boolean success = contract(stgParam.stg, contract).a.isEmpty();
			...
			if (!success) {
				...
				LinkedList<Integer> revertSignals = new LinkedList<Integer>();
				revertSignals.add(resultsSignal.getLast());
				
				//memorize last contracted signal
				Integer lastSignal = null;
				
				//needed to check whether it was sufficient to add the last signal
				...
				backtracking: while (true) {
					...
					stgParam.stg.setSignature(revertSignals, Signature.INPUT);
					...
					if (resultsSTG.size() == 1) {
						logging(stgParam.stg, DecompositionEvent.LAZY_REACHED_N, revertSignals);
						break backtracking;
					}
					...
					conflict: for (Integer actRevertSignal : revertSignals) {
						List<Set<Node>> parents = stgParam.stg.collectFromTransitions(
								ConditionFactory.getSignalOfCondition(actRevertSignal),
								CollectorFactory.getParentsCollector());
						
						Set<Node> sN = new HashSet<Node>();
						newConflict = false;
						for (Collection<Node> aP : parents)
							for (Node node : aP)
								if (!sN.add(node)) {
									newConflict = true;
									break conflict;
					...
					if (newConflict) {					
						...
						revertSignals.addLast(lastSignal);
